...
class CassandraIT extends FlatSpec with Matchers {
  ...
  "Bulk write" should "work" in {
    ...
    val cql = """
                ...
                |VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
              """.stripMargin
    val opts = CassandraOptions("scio", "table1", cql, "127.0.0.1")

    try {
      val sc = ScioContext()
      sc.parallelize(1 to n)
        .saveAsCassandra(opts)(toValues)
      sc.close()

      val result = cluster.connect().execute("SELECT * FROM scio.table1").asScala
      val expected = (1 to n).map(toValues)
      result.map(fromRow) should contain theSameElementsAs expected
    } finally {
      cluster.close()
...
object CassandraIT {
  ...
  def toValues(i: Int): Seq[Any] = Seq(
